...
@section('content')
    ...
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Carreras <small>Mantenimiento</small></h1>
    <!-- END page-header -->
    <!-- BEGIN row -->
    <div class="row">
        <!-- BEGIN col-12 -->
        <div class="col-xl-12">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-heading">
                    <h4 class="panel-title">Listado de Carreras</h4>
        ...
        $(function (){
            ...
            const $DataTable = $table.DataTable({
                ...
                "columns": [
                    ...
                    { title: "Descripción", data: "descripcion", className: "text-left" },
                    { title: "Fecha registro", data: "fecha_registro", className: "text-left"},
                    {
                        data: null,
                        defaultContent:
                            "<button type='button' class='btn btn-primary btn-xs btn-habilidad' data-toggle='tooltip' title='Habilidades'>Habilidades</button>",
                        "orderable": false,
                        "searchable": false,
                        "width": "26px"
                    },
                    {
                        data: null,
                        defaultContent:
                            "<button type='button' class='btn btn-info btn-xs btn-beneficio' data-toggle='tooltip' title='Beneficios'>Beneficios</button>",
                        "orderable": false,
                        "searchable": false,
                        "width": "26px"
                    },
                    {
                        data: null,
                        defaultContent:
                            "<button type='button' class='btn btn-success btn-xs btn-malla-curricular' data-toggle='tooltip' title='Malla Curricular'>Malla Curricular</button>",
                        "orderable": false,
                        "searchable": false,
                        "width": "26px"
                    },
            ...
            $table.on("click", ".btn-habilidad", function () {
                const id = $DataTable.row($(this).parents("tr")).data().id;
                getModal(`/auth/${$route}/partial-view-list-ability/${id ? id : 0}`);
            });

            $table.on("click", ".btn-beneficio", function () {
                const id = $DataTable.row($(this).parents("tr")).data().id;
                getModal(`/auth/${$route}/partial-view-list-gift/${id ? id : 0}`);
            });

            $table.on("click", ".btn-malla-curricular", function () {
                const id = $DataTable.row($(this).parents("tr")).data().id;
                getModal(`/auth/${$route}/partial-view-list-malla/${id ? id : 0}`);
            });
